...
import { Star, MessageSquare, Video, Quote, Building2, X } from "lucide-react";
...
const ClinicFeedback = () => {
  ...
  useEffect(() => {
    ...
    const params: Record<string, string> = { clinic_id: clinic.id };
  ...
  }, [clinic?.id, filterTestimonial]);

  const avgClinicRating =
    feedbacks.length > 0
      ? feedbacks.filter((f) => f.clinic_rating != null && f.clinic_rating > 0).length > 0
        ? feedbacks
            .filter((f) => f.clinic_rating != null && f.clinic_rating > 0)
            .reduce((s, f) => s + (f.clinic_rating || 0), 0) /
          feedbacks.filter((f) => f.clinic_rating != null && f.clinic_rating > 0).length
        : 0
      : 0;
  ...
  const withClinicRating = feedbacks.filter((f) => f.clinic_rating != null && f.clinic_rating > 0).length;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-heading font-bold text-foreground">Clinic feedback</h1>
          <p className="text-muted-foreground text-sm mt-1">
            Patient ratings and reviews for visits at this clinic. Includes doctor and clinic ratings and video testimonials.
          </p>
        </div>
      ...
      {feedbacks.length === 0 ? (
        <div className="glass-card rounded-xl p-12 text-center text-muted-foreground">
          ...
          <p className="text-sm mt-2">
            Feedback from patients who had appointments at this clinic will appear here.
          </p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="glass-card rounded-xl p-4 flex items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-2xl font-heading font-bold text-foreground">{avgDoctorRating.toFixed(1)}</span>
              </div>
              <p className="text-sm text-muted-foreground">Doctor rating ({feedbacks.length} reviews)</p>
            </div>
            <div className="glass-card rounded-xl p-4 flex items-center gap-4">
              <div className="flex items-center gap-2">
                <Star className="w-8 h-8 fill-amber-400 text-amber-500" />
                <span className="text-2xl font-heading font-bold text-foreground">
                  {withClinicRating > 0 ? avgClinicRating.toFixed(1) : "—"}
                </span>
              </div>
              <p className="text-sm text-muted-foreground">Clinic rating ({withClinicRating} reviews)</p>
            </div>
          </div>
            ...
            {feedbacks.map((f) => (
              ...
              >
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  <span className="text-sm font-medium text-foreground">{f.doctor_name}</span>
